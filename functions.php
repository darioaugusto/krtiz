...
function searchProductByTerm($search, $begin, $count, $order, $product_type_id) {
	...
	for ($i = 0; $i < count($words); $i++){
		...
		//$search_term = $search_term . '+(*' . trim($words[$i]) . "*) ";
		$search_term = $search_term . '+*' . trim($words[$i]) . "* ";
	}	
	
	//echo "select *, ifnull(release_date,home_release_date) as date from products where match(title) against ('" . $search_term . "' IN BOOLEAN MODE) and id_product_type = " . $product_type_id . " order by " . $order_by . " " . $limit_text;
	...
	if ($product_type_id > 0)
		...
		$result = mysql_query("select *, ifnull(release_date,home_release_date) as date, match(title, original_title) against ('" . $search_term . "' IN BOOLEAN MODE) as score from products where match(title, original_title) against ('" . $search_term . "' IN BOOLEAN MODE) and id_product_type = " . $product_type_id . " order by " . $order_by . " " . $limit_text);
		...
		$result = mysql_query("select *, ifnull(release_date,home_release_date) as date, match(title, original_title) against ('" . $search_term . "' IN BOOLEAN MODE) as score from products where match(title, original_title) against ('" . $search_term . "' IN BOOLEAN MODE) order by " . $order_by . " " . $limit_text);
	return $result;
}
